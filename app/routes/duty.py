...
def my_day():
    ...
    with get_connection() as conn:
        ...
        cursor.execute("""
            SELECT id, absence_type, status FROM absence
        ...
        if is_absent and absence_id:
            ...
            for row in cursor.fetchall():
                if row['is_mentor_duty']:
                    mentor_coverage = {
                        'substitute_name': row['substitute_name'],
                        'status': row['status']
                    }
                elif row['period_number']:
                    coverage_by_period[row['period_number']] = {
                        'substitute_name': row['substitute_name'],
                        'status': row['status'],
        ...
        # Sport duties at top (only show if not absent)
        if not is_absent:
        ...
        # Morning terrain duty (only show if not absent)
        if terrain_duty and not is_absent:
            schedule.append({
                'slot_type': 'duty',
        ...
        for slot in bell_slots:
            ...
            if slot['slot_type'] == 'register':
                ...
                if is_absent:
                    # Show coverage for absent teacher's periods
                    if p_num and p_num in coverage_by_period:
