...
def _author_desk_for_role(role):
    ...
    if role == 'activities':
        return 'Sport Office'
    # Any other flagged author (e.g. future admin) defaults to Management desk.
    return 'Management'


def _sniff_image(head, ext):
...
def board():
    ...
    items = "".join(
        "<li><strong>%s</strong> &middot; %s &middot; %s</li>" % (
            r['title'], r['category'], r['author_desk'])
        for r in rows
...
def new_notice():
    ...
    ext_claim = image.filename.rsplit('.', 1)[-1].lower() if '.' in image.filename else ''
    canon_ext = _sniff_image(img_bytes[:16], ext_claim)
    ...
    with get_connection() as conn:
        ...
        cur.execute(
            ...
            (notice_id, TENANT_ID, form['title'], body_val, form['category'],
             img_path, attachment_path, attachment_type, posted_by_id,
