            ...
            _cur.execute("""
                ...
                WHERE absence_id = ? AND request_date = ?
                  ...
                  AND period_id IN (
                      ...
                      WHERE sr2.absence_id = ? AND p.start_time >= ?
                  )
            """, (datetime.now().isoformat(), datetime.now().isoformat(),
                  absence_id, _today, absence_id, return_time))
...
def mark_back():
    ...
    # === E-03 Phase D: same-day PARTIAL period-level return (management) ===
    # Mirror of early_return's partial branch. If this is a single-day partial
    # absence being returned TODAY, release ONLY today's not-yet-started periods
    # (period.start_time >= server now). The absence stays live if earlier-today
    # periods remain covered. Return time is derived server-side (NOT the client
    # clock). Falls through to the existing day-granular path otherwise.
    _today = date.today().isoformat()
    _reported_by_id = session.get('staff_id') or 'management'
    with get_connection() as conn:
        _cur = conn.cursor()
        _cur.execute("""
            SELECT a.id, a.staff_id, a.absence_date, a.end_date, a.is_full_day,
        ...
        if _is_partial_today:
            ...
            if _remaining == 0:
                _cur.execute("""
                    ...
                    SET returned_early = 1, returned_at = ?, return_reported_by_id = ?,
                        status = 'Resolved', updated_at = ?
                    WHERE id = ?
                """, (datetime.now().isoformat(), _reported_by_id,
                      datetime.now().isoformat(), absence_id))
            else:
                _cur.execute("""
                    UPDATE absence
            ...
            # Log the partial return (management).
            _cur.execute("""
                INSERT INTO substitute_log (id, tenant_id, absence_id, event_type, staff_id, details, created_at)
                VALUES (?, ?, ?, 'early_return_partial', ?, ?, ?)
            """, (str(uuid.uuid4()), TENANT_ID, absence_id, _reported_by_id,
                  f'{{"return_time": "{return_time}", "released_requests": {released_count}, "remaining": {_remaining}, "marked_by": "management"}}',
                  datetime.now().isoformat()))
            ...
            return redirect('/substitute/overview')
    # === end Phase D partial branch (mgmt); day-granular path continues below ===
