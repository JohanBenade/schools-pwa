...
def apply_migration():
    ...
    cursor.execute(
        "UPDATE user_session SET can_post_notice = 1 WHERE role IN ('principal', 'deputy') AND is_active = 1"
    )
    print(f"Seeded can_post_notice for principal/deputy roles: {cursor.rowcount} rows")

    #    b) Delene by staff_id (role 'activities')
    cursor.execute(
        "UPDATE user_session SET can_post_notice = 1 WHERE staff_id = ? AND is_active = 1",
        (DELENE_STAFF_ID,),
    )
    delene_rows = cursor.rowcount
    print(f"Seeded can_post_notice for Delene (staff_id): {delene_rows} rows")
    if delene_rows == 0:
        # Delene is the sole non-role-derived author. A 0-row result means her
        # staff_id no longer matches an active session (reprovision/reseed) -
        # surface it loudly rather than reporting a silent success.
        print("WARNING: Delene seed matched 0 active rows - she will NOT be able "
              "to post notices. Verify her staff_id / is_active in user_session.")
