...
def apply_migration():
    ...
    try:
        ...
        # nullable. If not, raise - the single outer `except` below owns the
        # rollback+close (do NOT close here, or the outer cleanup hits an
        # already-closed connection and masks this error).
        cursor.execute("PRAGMA table_info(notice)")
        ...
        if post is None or post["notnull"] != 0:
            raise RuntimeError(
                ...
                "rolling back, schema_version NOT recorded."
            )
