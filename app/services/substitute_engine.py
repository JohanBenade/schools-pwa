...
def reassign_declined_request(request_id, declined_by_id):
    ...
    with get_connection() as conn:
        ...
        # Cycle day for the REQUEST's date. (Pre-v2 code always resolved to 1:
        # a dead 'if False' block followed by fetchone() on an exhausted cursor.
        # Correct cycle day is required for the room-occupancy check below and
                ...
                if sub.get('is_mentor_duty') and sub.get('mentor_group_id'):
                    ...
                    if cover:
                        ...
                        cursor.execute("""
                            ...
                            WHERE id = ?
                        """, (sub['id'],))
                        # Recalculate parent absence status based on all sub_requests
                        if sub.get('absence_id'):
                            cursor.execute("""
                                SELECT COUNT(*) as pending_count 
                                FROM substitute_request 
                                WHERE absence_id = ? AND status = 'Pending'
                            """, (sub['absence_id'],))
