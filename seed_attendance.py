...
anchor = cur.execute(
    "SELECT MAX(date) FROM attendance WHERE tenant_id = ?", (TENANT,)).fetchone()[0] or '0000-01-01'
...
days = [r['date'] for r in cur.execute(
    ...
    "ORDER BY date", (TENANT, anchor, today)).fetchall()]
# Idempotent: drop any date that already has attendance rows (this tenant).
days = [d for d in days if cur.execute(
    "SELECT COUNT(*) FROM attendance WHERE tenant_id = ? AND date = ?",
    (TENANT, d)).fetchone()[0] == 0]
if not days:
