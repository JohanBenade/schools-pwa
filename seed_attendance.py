...
today = (datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(hours=2)).date().isoformat()
anchor = cur.execute("SELECT MAX(date) FROM attendance").fetchone()[0]
...
assert len(groups) == 24, f"Expected 24 groups (25 minus 12 BD), got {len(groups)}"
